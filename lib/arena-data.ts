...
import type { AppRole, ArenaData, ArenaMatch, ArenaTeam, ArenaTournament, ArenaVenue } from "@/lib/types";
...
export async function getArenaData(): Promise<ArenaData> {
  ...
  try {
    ...
    const [rolesResult, tournamentsResult, venuesResult, teamsResult, playersResult, matchesResult] = await Promise.all([
